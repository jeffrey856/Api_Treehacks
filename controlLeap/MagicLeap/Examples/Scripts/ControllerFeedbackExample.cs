...
namespace MagicLeap
{
    ...
    public class ControllerFeedbackExample : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            startPosition = _dropdown.transform.position;
            startRotation = _dropdown.transform.rotation;
            print("dropdown positions: " + startPosition + " " + startRotation);
        ...
        private void HandleOnButtonUp(byte controllerId, MLInputControllerButton button)
        {
                ...
                button == MLInputControllerButton.Bumper)
            {
                ...
                bool cond1 = (Math.Abs(startPosition[0] - transform.position[0]) < epsilon) &&
                               (Math.Abs(startPosition[1] - transform.position[1]) < epsilon) &&
                                (Math.Abs(startPosition[2] - transform.position[2]) < epsilon);
                
                bool cond2 = (Math.Abs(startRotation[0] - transform.rotation[0]) < epsilon) &&
                                (Math.Abs(startRotation[1] - transform.rotation[1]) < epsilon) &&
                                (Math.Abs(startRotation[2] - transform.rotation[2]) < epsilon) &&
                                (Math.Abs(startRotation[3] - transform.rotation[3]) < epsilon);
                // bool cond2 = Math.abs(transform.rotation.Subtract(startRotation)) < epsilon;
                if (cond1 && cond2) {
                    print("COLLIDING!!");
                }
                else {
                ...
                // print("Controller's position (ButtonUp): " + transform.position);
                // print("Controller's rotation (ButtonUp): " + transform.rotation);
